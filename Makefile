...
test-cov: env-dev ## Run tests with coverage
	uv run pytest --cov=src/simple_email_gw --cov-report=term-missing $(TEST)
...
run: env-run ## Run the application
	uv run python -m simple_email_gw
...
pre-publish: check ## Pre-publication checks (run before publishing)
	...
	VERSION_INIT=$$(grep '^__version__ = ' src/simple_email_gw/__init__.py | cut -d'"' -f2); \
...
# - email-gw-mcp-server: Run the MCP server (was: uv tool run --with . email-gw-mcp-server)
